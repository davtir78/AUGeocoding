...
# Get columns for mmm
print("\nMMM Columns:")
sql = "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'mmm' ORDER BY ordinal_position;"
res = execute_statement(sql)
for row in res['records']:
    print(f" - {row[0]['stringValue']}: {row[1]['stringValue']}")

# Get columns for mesh_block
print("\nMesh Block Columns:")
sql = "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'mesh_block' ORDER BY ordinal_position;"
res = execute_statement(sql)
for row in res['records']:
    print(f" - {row[0]['stringValue']}: {row[1]['stringValue']}")
