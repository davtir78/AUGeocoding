...
exports.fetchRevisionQuizJobResponse = void 0;
 ...
 */
exports.fetchRevisionQuizJobResponse = (0, firestore_1.onDocumentCreated)({
    ...
    if (!snapshot) {
        v2_1.logger.error('[fetchRevisionQuizJobResponse] No snapshot provided for job:', jobId);
        return;
    }
    const job = snapshot.data();
    if (!job) {
        v2_1.logger.error('[fetchRevisionQuizJobResponse] Job data missing for job:', jobId);
        return;
    }
    ...
    v2_1.logger.info('[fetchRevisionQuizJobResponse] Triggered for job:', jobId, 'data:', job);
    if (job.status !== 'queued') {
        v2_1.logger.info('[fetchRevisionQuizJobResponse] Skipping job (status not queued):', jobId, job.status);
        return;
    }
    if (!OPENROUTER_API_KEY) {
        v2_1.logger.error('[fetchRevisionQuizJobResponse] OPENROUTER_API_KEY is not set in environment. Cannot call OpenRouter.');
    ...
    if ((!topic || !topic.trim()) && !hasKnowledgeItems) {
        v2_1.logger.error('[fetchRevisionQuizJobResponse] Missing or invalid topic/items in job:', jobId);
        await snapshot.ref.update({
            status: 'fetch_failed',
    ...
    if (hasKnowledgeItems) {
        // --- 1-to-1 MAPPING MODE ---
        const itemContexts = knowledgeItems.map((item, i) => {
...
`.trim();
    ...
    v2_1.logger.info('[fetchRevisionQuizJobResponse] Starting OpenRouter call for job:', jobId, {
        model: modelToUse,
        topic: trimmedTopic,
    });
    try {
        ...
        v2_1.logger.error('[fetchRevisionQuizJobResponse] Failed to update job to fetching:', jobId, e);
        return;
    }
    let contentText = '';
    try {
        ...
        v2_1.logger.error('[fetchRevisionQuizJobResponse] All models failed:', e);
        await snapshot.ref.update({
            status: 'fetch_failed',
    ...
    v2_1.logger.info('[fetchRevisionQuizJobResponse] Received JSON text length:', contentText.length);
    await snapshot.ref.update({
    ...
    v2_1.logger.info('[fetchRevisionQuizJobResponse] Job marked as fetched:', jobId);
});
