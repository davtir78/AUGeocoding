...
exports.reprocessPresentations = void 0;
...
function normalizeSlidePayload(rawSlide, defaultBaseHref) {
    ...
    try {
      ...
      resizeSlide();
        ...
        v2_1.logger.warn('[reprocess] Server-side DOMPurify unavailable or failed; using scaffolded HTML as safeHtml fallback');
        safeHtml = scaffolded;
    }
    return {
        html: scaffolded,
 ...
 * { "limit": 10 }
 */
exports.reprocessPresentations = (0, https_1.onRequest)(async (req, res) => {
    try {
        ...
        if (presentationId) {
            ...
            if (structured && Array.isArray(structured.slides)) {
                formattedSlides = structured.slides.map((s) => {
                ...
                formattedSlides = parts.map((p, idx) => {
        ...
        v2_1.logger.error('[reprocess] Unexpected error', err);
