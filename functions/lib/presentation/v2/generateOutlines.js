...
exports.generateOutlines = generateOutlines;
const utils_1 = require("./utils");
const schemas_1 = require("./schemas");
/**
 * Generates the presentation outline (slides content) using an LLM.
 */
async function generateOutlines(topic, nSlides, language = 'English', additionalContext, tone, verbosity, instructions, includeTitleSlide = true, ageGroup = 'late-primary', model = 'openai/gpt-4o-mini') {
    const systemPrompt = `
    ...
    const response = await (0, utils_1.callOpenRouter)(model, messages, schemas_1.PresentationOutlineSchema // This triggers response_format: { type: "json_object" } inside utils
    );
    // Validate with Zod
    const parsed = schemas_1.PresentationOutlineSchema.parse(response);
