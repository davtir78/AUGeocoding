...
exports.generateSlideContent = generateSlideContent;
const zod_to_json_schema_1 = require("zod-to-json-schema");
const zod_1 = require("zod");
const utils_1 = require("./utils");
const layouts_1 = require("./layouts");
const model_config_1 = require("../../config/model-config");
async function generateSlideContent(slideOutline, layoutId, language = "English", tone = "Educational", verbosity = "Standard", ageGroup = "late-primary", model = (0, model_config_1.getDefaultModelForType)(model_config_1.ModelType.General)) {
    const layout = layouts_1.LAYOUTS.find(l => l.id === layoutId);
    if (!layout) {
        throw new Error(`Layout ${layoutId} not found`);
    }
    // Convert Zod schema to JSON schema for the prompt
    // WRAPPER: specific for Orator's Podium (Sprint 3.5)
    const wrappedSchema = zod_1.z.object({
        visualContent: layout.schema,
        detailedNotes: zod_1.z.string().describe("Academic, detailed paragraph explaining the slide's content in depth (for 'Read More'). ~100-150 words."),
        narrationScript: zod_1.z.string().describe("Conversational, first-person narration script for this slide (~40-60 words). Spoken by a friendly tutor.")
    });
    const jsonSchema = (0, zod_to_json_schema_1.zodToJsonSchema)(wrappedSchema, "slideGeneration");
    const systemPrompt = `
    ...
    const response = await (0, utils_1.callOpenRouter)(model, messages, wrappedSchema);
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // Find Bullet Layout
                const bulletLayout = layouts_1.LAYOUTS.find(l => l.id === "bullet-points-slide");
                if (bulletLayout) {
                    ...
                    // Fallback notes
                    if (!detailedNotes)
                        detailedNotes = bullets.join(". ");
                    if (!narrationScript)
                        narrationScript = `Here are the key points about ${title}. ` + bullets.join(". ");
                }
                else {
                ...
                throw new Error(`Zod Error: ${retryError.message} | Fallback Failed: ${fallbackError.message} | Raw: ${JSON.stringify(response)} `);
            }
        }
    }
