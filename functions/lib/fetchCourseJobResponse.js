...
exports.fetchCourseJobResponse = void 0;
 ...
 */
exports.fetchCourseJobResponse = (0, firestore_1.onDocumentCreated)({
    ...
    if (!snapshot) {
        v2_1.logger.error('[fetchCourseJobResponse] No snapshot provided for job:', jobId);
        return;
    }
    const job = snapshot.data();
    if (!job) {
        v2_1.logger.error('[fetchCourseJobResponse] Job data missing for job:', jobId);
        return;
    }
    v2_1.logger.info('[fetchCourseJobResponse] Triggered for job:', jobId, 'data:', job);
    try {
        // Idempotency: Fetch fresh document using existing snapshot ref
        const currentSnap = await snapshot.ref.get();
        if (!currentSnap.exists)
            return;
        const freshJob = currentSnap.data();
        if (freshJob.type !== 'course') {
            v2_1.logger.info('[fetchCourseJobResponse] Skipping job (not type=course):', jobId, freshJob.type);
            return;
        }
        if (freshJob.status !== 'queued') {
            v2_1.logger.info('[fetchCourseJobResponse] Skipping job (status not queued - IDEMPOTENCY):', jobId, freshJob.status);
            return;
        }
        if (!OPENROUTER_API_KEY) {
            v2_1.logger.error('[fetchCourseJobResponse] OPENROUTER_API_KEY is not set in environment. Cannot call OpenRouter.');
        ...
        if (!topic || !topic.trim()) {
            v2_1.logger.error('[fetchCourseJobResponse] Missing or invalid topic in job:', jobId);
            await snapshot.ref.update({
                status: 'fetch_failed',
...
`.trim();
        v2_1.logger.info('[fetchCourseJobResponse] Starting OpenRouter call for job:', jobId, {
            model: modelToUse,
            topic: trimmedTopic,
        });
        try {
            ...
            v2_1.logger.warn('[fetchCourseJobResponse] Failed to update job to fetching (race condition?):', jobId, e);
            return;
        }
        let contentText = '';
        try {
            const models = Array.isArray(modelToUse) ? modelToUse : [modelToUse];
            const response = await (0, openrouter_1.callOpenRouterWithFallback)(models, [
                ...
                response_format: {
                    ...
                    json_schema: {
                        name: 'lesson_draft',
                        strict: true,
                        schema: schemas_1.LessonDraftSchema
                    }
                }
            });
            ...
            v2_1.logger.error('[fetchCourseJobResponse] All models failed:', e);
            await snapshot.ref.update({
                status: 'fetch_failed',
        ...
        v2_1.logger.info('[fetchCourseJobResponse] Received course JSON text length:', contentText.length);
        await snapshot.ref.update({
            status: 'fetched',
        ...
        v2_1.logger.info('[fetchCourseJobResponse] Job marked as fetched:', jobId);
    }
    catch (err) {
        v2_1.logger.error('[fetchCourseJobResponse] Unexpected error:', jobId, err);
        try {
            await snapshot.ref.update({
                status: 'fetch_failed',
            ...
            v2_1.logger.error('[fetchCourseJobResponse] Failed to set error status:', jobId, innerErr);
        }
    }
});
