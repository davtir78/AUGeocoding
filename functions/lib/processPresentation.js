...
function extractFinalContentFromSse(responseText) {
    v2_1.logger.info('[DEBUG] Attempting to extract content from SSE stream');
    ...
    for (const line of lines) {
        if (line.startsWith('data: ')) {
            try {
                ...
                v2_1.logger.warn('[DEBUG] Failed to parse SSE data line:', e instanceof Error ? e.message : String(e));
            }
        }
    }
    v2_1.logger.info(`[DEBUG] Extracted ${concatenatedText.length} characters from SSE stream.`);
    return concatenatedText.trim();
}
// Extract any presentation content from the raw response text (Heuristic fallback)
function extractAnyPresentationContent(responseText) {
    v2_1.logger.info('[DEBUG] Attempting to extract presentation content from raw response using heuristic fallback');
    ...
    if (textContent.length > 200) {
        v2_1.logger.info(`[DEBUG] Using cleaned text content as fallback: ${textContent.length} characters`);
        return textContent;
    }
    return '';
}
// Create a basic presentation as a fallback
function createBasicPresentation(title, slideCount) {
    v2_1.logger.info(`[DEBUG] Creating basic presentation for "${title}" with ${slideCount} slides`);
    let content = `# ${title}\n\n`;
...
function parseZaiResponse(content) {
    const slides = [];
...
function scaffoldHtml(fragment, baseHref) {
    const headExtras = [];
    if (baseHref)
        headExtras.push(`<base href="${baseHref}">`);
    const defaultStyles = `
    <style>
...
function normalizeSlidePayload(rawSlide, defaultBaseHref) {
    ...
    try {
        ...
        if (!slideDiv) {
            v2_1.logger.info('[DEBUG] No .slide wrapper found. Wrapping content in .slide div.');
            const body = doc.body;
            if (body) {
        ...
        v2_1.logger.warn('[DEBUG] Failed to enforce .slide wrapper:', err);
    ...
    try {
      ...
      setTimeout(resizeSlide, 100); 
        ...
        v2_1.logger.warn('[DEBUG] Server-side DOMPurify unavailable or failed; using scaffolded HTML as safeHtml fallback');
        safeHtml = scaffolded;
    }
    return {
        html: scaffolded,
...
exports.processPresentation = (0, firestore_1.onDocumentUpdated)({
    ...
    v2_1.logger.info('[DEBUG] processPresentation triggered');
    ...
    const processingPromise = async () => {
        ...
        v2_1.logger.info('[DEBUG] Before data:', beforeData);
        v2_1.logger.info('[DEBUG] After data:', afterData);
        ...
        if (beforeData?.status !== 'fetched' && afterData?.status !== 'fetched') {
            v2_1.logger.info(`[DEBUG] Skipping presentation ${presentationId} - status change not from/to 'fetched': ${beforeData?.status} → ${afterData?.status}`);
            return;
        }
        // Only process when status changed TO 'fetched'
        if (afterData?.status !== 'fetched') {
            v2_1.logger.info(`[DEBUG] Skipping presentation ${presentationId} - new status is not 'fetched': ${afterData?.status}`);
            return;
        }
        const { title, slideCount, theme, userId, rawContent, bucketUrl } = afterData;
        v2_1.logger.info(`Starting presentation processing for ${presentationId} for user ${userId}`);
        v2_1.logger.info(`[DEBUG] Raw content length: ${rawContent?.length || 0}`);
        v2_1.logger.info(`[DEBUG] Bucket URL: ${bucketUrl}`);
        // Use the raw content that was already fetched and processed
        let contentToProcess = rawContent;
        if (!contentToProcess) {
            v2_1.logger.error('[DEBUG] No raw content found in presentation document');
            try {
                ...
                v2_1.logger.error('[DEBUG] Failed to update presentation with error', updateError instanceof Error ? updateError.message : String(updateError));
            }
            return;
        }
        ...
        try {
            v2_1.logger.info('[DEBUG] Starting to process fetched content');
            let formattedSlides = [];
            ...
            if (contentToProcess.includes('extractionMethod') && contentToProcess.includes('sse_parsing')) {
                v2_1.logger.info('[DEBUG] Processing structured content from SSE parsing');
                try {
                    const structuredData = JSON.parse(contentToProcess);
                    if (structuredData.slides && Array.isArray(structuredData.slides)) {
                        v2_1.logger.info(`[DEBUG] Found ${structuredData.slides.length} structured slides from SSE parsing`);
                        // The slides are already properly structured from the SSE parsing
                        formattedSlides = structuredData.slides.map((slide) => {
                        ...
                        v2_1.logger.info(`[DEBUG] Processed ${formattedSlides.length} slides from SSE parsing`);
                    }
                }
                catch (parseError) {
                    v2_1.logger.warn('[DEBUG] Failed to parse SSE structured content, falling back to text parsing:', parseError.message);
                }
            }
            // Fallback: If no structured content found, use existing text parsing
            if (formattedSlides.length === 0) {
                v2_1.logger.info('[DEBUG] No structured content found, using fallback text parsing');
                ...
                if (contentToProcess.includes('data: {') && contentToProcess.includes('"phase":')) {
                    v2_1.logger.info('[DEBUG] Raw content detected as SSE stream. Using dedicated parser.');
                ...
                v2_1.logger.info(`[DEBUG] Parsed ${parsedSlides.length} slides from fetched content`);
            ...
            v2_1.logger.info(`[${presentationId}] Successfully processed and stored ${formattedSlides.length} slides.`);
            ...
            if (generationJobId) {
                try {
                    ...
                    v2_1.logger.info(`[${presentationId}] Updated parent generationJob ${generationJobId} to completed.`);
                }
                catch (jobUpdateErr) {
                    v2_1.logger.error(`[${presentationId}] Failed to update parent generationJob ${generationJobId}:`, jobUpdateErr);
                    // Don't fail the presentation processing just because the job update failed
                }
            }
        }
        catch (e) {
            v2_1.logger.error('Error during presentation processing', e);
