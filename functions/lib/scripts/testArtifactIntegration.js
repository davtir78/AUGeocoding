        ...
        if (status === 'completed') {
            ...
            if (itemsSnapshot.empty) {
                ...
                itemsSnapshot.forEach(doc => {
                    ...
                    if (!data.year_level)
                        missing.push('year_level');
                    if (!data.strand)
                        missing.push('strand');
                    if (!data.extracted_text)
                        missing.push('extracted_text');
                    if (!data.student_answer && data.student_answer !== "")
                        missing.push('student_answer');
                    if (!data.ideal_answer)
                        missing.push('ideal_answer');
                    if (!data.explanation)
                        missing.push('explanation');
                    if (!data.crop_image_url)
                        missing.push('crop_image_url');
                    if (!data.source_image_url)
                        missing.push('source_image_url');
                    if (!data.curriculum_links || data.curriculum_links.length === 0 || !data.curriculum_links[0].code)
                        missing.push('curriculum_links[0].code');
                    if (missing.length > 0) {
