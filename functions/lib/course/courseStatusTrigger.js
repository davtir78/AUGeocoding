...
exports.syncCourseStatus = void 0;
 ...
 */
exports.syncCourseStatus = (0, firestore_1.onDocumentUpdated)({
    ...
    if (!courseId)
        return; // Presentation not part of a course
    firebase_functions_1.logger.info(`[syncCourseStatus] Presentation ${presentationId} finished (${after.status}). Checking Course ${courseId}...`);
    ...
    firebase_functions_1.logger.info(`[syncCourseStatus] Course ${courseId} stats: ${siblingsSnap.size} total, ${pendingCount} pending, Failed: ${anyFailed}, AllCompleted: ${allCompleted}`);
    const courseRef = db.collection('courses').doc(courseId);
    if (allCompleted) {
        // All Done -> READY
        firebase_functions_1.logger.info(`[syncCourseStatus] Marking Course ${courseId} as READY.`);
        ...
        firebase_functions_1.logger.warn(`[syncCourseStatus] Marking Course ${courseId} as FAILED (partial).`);
        await courseRef.update({
            status: 'error',
