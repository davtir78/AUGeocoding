...
import { ScoutResult, ScoutJSONSchema } from './schemas';
...
export class ScoutClient {
     ...
     */
    async detectLayoutAndPII(imageUrl: string, userHint?: string): Promise<{ result: ScoutResult, safeImageUrl: string }> {
        logger.info(`[Scout] analyzing ${imageUrl} with hint: ${userHint || 'none'}`);
        ...
        try {
            const llmResult = await callWithSafetyFallback(
                ...
                ModelType.Vision, // Fallback to standard Vision (Claude/GPT-4o)
                [
            ...
            if (!llmResult) {
                logger.warn('[Scout] Layout analysis blocked by safety filters (Double Refusal).');
                throw new Error('SAFETY_BLOCK: Document content triggered safety filters.');
            }

            // Parse result
            const content = llmResult.choices[0].message.content;
            const parsed: ScoutResult = JSON.parse(jsonrepair(content));
            ...
            if (piiRegions.length > 0) {
                logger.info(`[Scout] PII Detected in ${piiRegions.length} regions. Blurring...`);
            ...
            logger.error('[Scout] Analysis failed', error);
            throw error;
        }
    }
     ...
     */
    private async blurPII(imageUrl: string, bboxes: [number, number, number, number][]): Promise<string> {
        try {
            // 1. Download
            ...
            logger.error('[Scout] PII Blurring failed', e);
